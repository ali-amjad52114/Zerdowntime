...
function handoffPanel(handoff, selectedIds) {
  ...
  return `<section class="handoff" aria-labelledby="handoff-title">
    ...
    <div class="controls"><button type="button" class="secondary" id="save-selection">Save human selection</button><button type="button" class="secondary" id="analyze-target">Analyze Target</button><button type="button" id="handoff-selection"${selectedIds.size ? '' : ' disabled'}>Hand off to X/Y/Z</button></div>
  </section>`;
}
...
function diligenceResults(handoff) {
  ...
  return `<section class="diligence-results" aria-labelledby="diligence-results-title">
    <div class="diligence-head"><div><div class="eyebrow">TARGET DILIGENCE RESULTS</div><h2 id="diligence-results-title">X/Y/Z evidence after human selection</h2></div><button type="button" class="secondary" id="analyze-target-diligence">Analyze Target</button></div>
    ${results.map((result) => `<article class="diligence-target" data-diligence-target="${escapeHtml(result.target ?? result.candidate_id)}" data-uniprot-id="${escapeHtml(result.uniprot_id ?? result.uniprotId ?? '')}"><h3>${escapeHtml(result.target ?? result.candidate_id)}</h3>
...
function resolveUniprot(target, uniprotId) {
  ...
  return String(target ?? '').toUpperCase().includes('SERPINA1') ? 'P01009' : '';
}
...
async function renderStructureViews(structure) {
  const viewer = getViewer();
  await loadStructureModel(viewer, structure.structure_url);
  const pocket = (structure.pockets ?? [])[0] ?? {};
...
function renderAnalysis(analysis, images) {
  ...
  const pockets = (structure.pockets ?? []).slice(0, 3);
  ...
  panel.innerHTML = '<div class="candidate-head"><div><div class="eyebrow">TARGET STRUCTURE</div><h2 id="target-analysis-title">Analyze Target</h2></div>' + predicted + '</div>'
    ...
    + '</div>'
    + '<h3>Pockets</h3><ul class="pocket-list">' + pocketItems + '</ul>'
    + '<div class="controls"><button type="button" disabled>Investigate known compounds for Pocket #1</button></div>';
}
async function analyzeTarget() {
  try {
    ...
    const images = await renderStructureViews(analysis.structure);
...
document.querySelector('#analyze-target')?.addEventListener('click', analyzeTarget);
document.querySelector('#analyze-target-diligence')?.addEventListener('click', analyzeTarget);
</script></body></html>`;
}
