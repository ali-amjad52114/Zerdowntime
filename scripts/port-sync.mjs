...
for (const name of ["PORT_CLIENT_ID", "PORT_CLIENT_SECRET", "PORT_GITHUB_ORG", "PORT_GITHUB_REPO"]) {
...
for (const relative of manifest.actions) {
  const action = JSON.parse(readFileSync(join(root, relative), "utf8"), (key, value) => value === "REPLACE_WITH_GITHUB_ORG" ? process.env.PORT_GITHUB_ORG : value === "REPLACE_WITH_GITHUB_REPO" ? process.env.PORT_GITHUB_REPO : value);
