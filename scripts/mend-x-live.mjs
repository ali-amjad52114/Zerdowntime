...
const run = runXAxis({
  snapshot: { pipeline_items: rows, source_url: target, retrieved_at: new Date().toISOString() },
  mode: 'normal',
  adapterVersion: 'v1',
...
if (run.validation.status !== 'PASS') {
  console.error(JSON.stringify({ rawKeys: rows.map((row) => Object.keys(row ?? {})) }, null, 2));
  process.exit(1);
}
